...
const setTodos = () => {
  const todos = [
    ...
    {
      id: 8, task: 'Regular Expressions', due: '', done: false, details: 'Regular Expression literals and objects, syntax, methods, string regexp methods.',
    },
  ];
  let listItems = '';
  todos.forEach((todo) => {
...
console.log("1 Use regular expressions to escape HTML tags by replacing < and > with their HTML entity equivalents &lt; and &gt; \nconst task = task.value.trim().replace(/</g, '&lt;').replace(/>/g, '&gt;');");
console.log("2 const details = details.value.trim().replace(/</g, '&lt;').replace(/>/g, '&gt;');");
