...
console.log(1, 'Check if there is an internet connection: navigator.onLine', navigator.onLine);
try {
  if (!navigator.onLine) {
...
console.log(`2 If there is not an internet connection you can display a message without creating an exception.

if (!navigator.onLine) {
  const errMsgElem = document.createElement('li');
  errMsgElem.textContent = 'No Internet Connection Error.';
  list.insertAdjacentElement('beforebegin', errMsgElem);
}`);
console.log(`3 If there is not an internet connection you can throw an exception  and display the message. Use a try catch block to catch the exception.

try {
  if (!navigator.onLine) {
    throw 'No Connection. Bootstrap and Bootstrap icon CDNs require an internet connection.'
  }
} catch (err) {
  const errMsgElem = document.createElement('li');
  errMsgElem.textContent = err;
  list.insertAdjacentElement('beforebegin', errMsgElem);
}`);

console.log(`4 The exception thrown can be an error object. Pass in the message as the argument.
