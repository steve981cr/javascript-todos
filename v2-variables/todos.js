...
console.log(1, `const addbtn = document.getElementById('add-btn'); // HTML button elements are assigned to const variables.`);
console.log(2, `const task = document.getElementById('task'); // Add ToDo HTML form and list elements are assigned to const variables.`);
console.log(3, `const details = document.getElementById('details');`);
...
const setTodos = () => {
  const todos = [
    ...
    {
      id: 2, task: 'Literals and Variables', due: '', done: false, details: 'Literals, primitive vs object variables, const and let vs var, scope, destructuring assignment',
    },
  ];
  let listItems = '';
  todos.forEach((todo) => {
...
console.log(4, 'let id = 0; // The id variable is set to a number literal. \nThe value will change so use the let keyword.');
// Handle Add Todo form submission.
submitBtn.addEventListener('click', () => {
  id = id + 1;
  console.log(5, 'id = id + 1; // Add 1 to the id variable every time a new ToDo is submitted.');
  const todo = { id: id, task: task.value, details: details.value, done: false };
  console.log(6, 'const todo = { id: id, task: task.value, details: details.value, done: false }; // Assign todo to an object literal. Objects are assigned to const variables.');
  console.log(7, 'Todo object after the new ToDo form is submitted:', todo);
...
const todo = {};
console.log(8, 'const todo = {}; // Set the todo variable to an object literal. \nScope the variable is outside the click event callback function so the variable does not get reset on each click event.');
// Handle clicks on todo items.
list.addEventListener('click', (e) => {
  const li = e.target.parentElement;
  const { id } = li;
  console.log(9, 'const { id } = li; // Object destructuring assignment, equivalent to const id = li.id;');
  todo.id = id;
  ...
  if (e.target && e.target.hasAttribute('data-done')) {
    ...
    console.log(10, 'Todo object after pressing the edit icon:', todo);
    ...
    console.log(11, 'Todo object after submitting the edit form:', todo);
    ...
    console.log(12, 'Todo object when edit form is cancelled:', todo);
