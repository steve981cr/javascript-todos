...
const createLi = (todo, showDetails = false) => {
  ...
  console.log(1, 'todo.due.value', todo.due.value, 'HTML Date format fields return dates in YYYY-MM-DD format.');  
  console.log(2, "new Date(todo.due).toLocaleDateString(undefined, { month: 'short', day: 'numeric', timeZone: 'UTC' })", new Date(todo.due).toLocaleDateString(undefined, { month: 'short', day: 'numeric', timeZone: 'UTC' }), 'Convert the date string to a date object. Then use the toLocaleDateString method to display the date using the computer\'s language settings. Display the month and day using the UTC time zone.');
...
const setTodos = () => {
  const todos = [
    ...
    {
      id: 10, task: 'Dates', due: '', done: false, details: 'Number literals, Math Global Object.',
    },
  ];
  let listItems = '';
  todos.forEach((todo) => {
...
submitBtn.addEventListener('click', () => {
  id += 1;
  console.log(3, 'due.value', due.value, 'Get the input date field value from the add todo form');
...
list.addEventListener('click', (e) => {
  ...
  if (e.target && e.target.hasAttribute('data-done')) {
    ...
    console.log(4, 'new Date(`${li.children[5].textContent}, ${new Date().getFullYear()}`).toISOString().substr(0, 10', new Date(`${li.children[5].textContent}, ${new Date().getFullYear()}`).toISOString().substr(0, 10), 'Convert the todo item due date to a date object. Then convert the date object to YYYY-MM-DD format.');
    ...
    console.log(5, 'editDue.value', editDue.value); 
