...
const setTodos = () => {
  const todos = [
    ...
    {
      id: 11, task: 'Arrays', due: '', done: false, details: 'Creating arrays, static and instance properties and methods, shallow copies, sets.',
    },
  ];
  console.log(`1 Create an array literal of ToDo objects. 
const todos = [
  { id: 1, task: 'General', due: '', done: true, details: 'Lorem ipsum' },
  { id: 2, task: 'Variables', due: '', done: true, details: 'Lorem ipsum' },
  { id: 3, task: 'Operators', due: '', done: true, details: 'Lorem ipsum' },
]`);

  // Sort ToDos by Id number in descending order.
  todos.sort((a, b) => b.id - a.id);
  console.log('2 Sort by ToDo id in descending order. \ntodos.sort((a, b) => b.id - a.id);');  
  ...
  console.log(`3 Use the forEach method to iterate through each item in the array:
todos.forEach((todo) => {
  listItems += \`<li id="\${todo.id}" class="list-group-item">\${createLi(todo)}</li>\`;
});`);
  list.innerHTML = listItems;
  id = todos.length;
  console.log(4, 'id = todos.length; // The id variable is set to the length of the todos array (i.e., the number of items');
}
setTodos();
